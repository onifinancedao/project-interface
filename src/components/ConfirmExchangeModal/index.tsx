...
import { useWithdrawUtilityTokensCallback } from "../../state/exchange/hooks"
    ...
    <div className={"col-12 my-2 py-2 token-item " + theme}>
      <div className="row">
         ...
         <div className="col-6 d-grid align-content-center text-center">
            ...
            <small className='text-success'>+ {CurrencyAmount.fromRawAmount(UTILITY_TOKEN, amount.toString()).toSignificant()} {UTILITY_TOKEN.symbol}</small>
         </div>
...
export default function ConfirmExchangeModal({list, setList, amount}:{list:JSBI[], setList: Dispatch<SetStateAction<JSBI[]>>, amount:JSBI}){
    const [darkMode, ] = useDarkModeManager()
    const isMobile = useIsMobile()
  
    const isOpen = useModalIsOpen(ApplicationModal.EXCHANGE)
    const closeModal = useCloseModal(ApplicationModal.EXCHANGE)
    ...
    const transactionCallback = useWithdrawUtilityTokensCallback()
...
const pendingText = <Trans>Exchanging {list.length} {PROJECT_TOKEN.symbol} tokens for {CurrencyAmount.fromRawAmount(UTILITY_TOKEN, JSBI.multiply(amount, JSBI.BigInt(list.length)).toString()).toSignificant()} {UTILITY_TOKEN.symbol} tokens.</Trans>
                ...
                <div className="container">
                    <div className="row">
                          ...
                          <div className="row">
                            ...
                            <div className="col-12 "style={{overflow:"auto", maxHeight: "calc(85vh - 150px)"}}>
                              ...
                              >
                                ...
                                <span>{CurrencyAmount.fromRawAmount(UTILITY_TOKEN, JSBI.multiply(amount, JSBI.BigInt(list.length)).toString()).toSignificant()} {UTILITY_TOKEN.symbol}</span>
                              </div>
                              <div className="col-12">
                                <ActionButton onClick={handleTransaction}><Trans>Exchange {list.length} {PROJECT_TOKEN.symbol}</Trans></ActionButton>
                              </div>
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="col-12 my-4 text-center">
                          <Trans>Add items to exchange.</Trans>
                        </div>
                        }
                    </div>
                </div>
